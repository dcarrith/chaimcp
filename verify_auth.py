...
def run_test():
    # Start port-forward
    pf = subprocess.Popen(["kubectl", "port-forward", "svc/chiamcp", "8000:8000"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    time.sleep(3) # Wait for connection

    try:
        print("Testing without token...")
        try:
            res = subprocess.run(["curl", "-i", "-s", "-o", "/dev/null", "-w", "%{http_code}", "http://localhost:8000/sse"], capture_output=True, text=True, timeout=5)
        ...
        try:
            ...
            subprocess.run(["curl", "-i", "-H", "Authorization: Bearer my-secret-token", "http://localhost:8000/sse"], timeout=2, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
